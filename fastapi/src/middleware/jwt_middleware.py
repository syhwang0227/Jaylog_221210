...
class JwtMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:  # request: 프론트에서 들어온 요청이 담겨 있음
        ...
        if 'authorization' in request.headers.keys():
            ...
            try:
                decoded_jwt = jwt.decode(access_token, constants.JWT_SALT, algorithms=['HS256'])
